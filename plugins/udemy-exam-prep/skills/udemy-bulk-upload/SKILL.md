...
**一括アップロードは、そのテストの既存の問題をすべて CSV の内容で置き換える（追加ではない）。**
置き換わるのは編集中の版（下書き）で、「変更内容を公開」を押すまで受講者に見える版は変わらない。
テストの quizId・タイトル・設定値（時間・合格点・ランダム化）はそのまま残る。

> 実績: 50問のテストに検証用の2問だけの CSV を投入したら、編集画面の問題が2問だけになった。
> 公開版（API の `/api-2.0/quizzes/<id>/assessments/`）は50問のままだった。正規の CSV を投入し直して戻した。
> **検証用の CSV を本番のテストに投入しない。** 投入したら公開せずに正規の CSV を入れ直す。

**出典 URL は `linkify_csv.py` でリンクにした投入用 CSV を使う**（下記「出典 URL はリンクにして投入する」）。
...
### 出典 URL はリンクにして投入する（必須）

**素の URL のまま一括アップロードすると、テキストとして保存され受講者はクリックできない。**
**CSV の出典を `<a href="URL">URL</a>` にして投入すればリンクとして保存される。**
quiz.csv（Source of Truth）は素の URL のまま残し、投入の直前に `linkify_csv.py` で投入用の CSV を別に作る。

```bash
python "${CLAUDE_PLUGIN_ROOT}/scripts/linkify_csv.py" section0*/quiz.csv --out-dir .upload
python "${CLAUDE_PLUGIN_ROOT}/scripts/validate_quiz_csv.py" .upload/*/quiz.csv   # <a href> はタグ検査の対象外
# → .upload/<セクションフォルダ>/quiz.csv を投入する（.upload は .gitignore に入れる）
```

> 実績（公開中の講座で確認）:
> - 素の URL は `<p>出典: https://learn.example.com/...</p>` として保存された（`<a>` なし）
> - CSV に `<a href="URL">URL</a>` と書くと `<a href="URL" target="_blank" rel="noopener noreferrer">` として保存された（`target` は Udemy が付ける）
> - Markdown の `[text](url)` は変換されず、文字のまま残った
> - 300問・320リンクを投入し、本文（innerText）が quiz.csv と完全一致、素の出典 URL は0件だった

- `linkify_csv.py` がリンクにするのは **「出典:」「参考:」で始まる行の URL だけ**。本文中のスコープ値（`https://…/.default`）や audience 値のような「URL の形をした値」はリンクにしない。URL に使える半角文字だけを取るので、直後の日本語を巻き込まない（実績: 全角を許す正規表現だと `https://…/.defaultのOAuth` までリンクになった）
- 表示される文字は変わらないので、編集画面ヘルパーの署名照合は quiz.csv のデータのまま使える
- 投入済みで素の URL が残っている場合は、エディタ上でリンクにできる（ヘルパーの `'link'` モード）:

```
1. 同期サーバーを起動し（下記「多数の問題を書き換える」の手順1）、編集画面でヘルパーを読み込んで __u.load(<セクション番号>)
2. __u.runBg([1, 2, ..., N], 'link')   // 本文には触れず、出典行の素の URL だけを <a target="_blank"> にして保存
3. 再読み込み → ヘルパー再読み込み → __u.load(N) → __u.auditBg(1, N)
   auditStatus() の mismatch が空なら、本文が CSV と一致し、かつ素の出典 URL が残っていない（照合の unlinked が 0）
```

- `run`（書き換え）も入力の直後に出典行の URL をリンク化する（`__u.links = false` で無効化）
- 手作業ならエディタの「リンクを挿入」（`data-purpose="PROMPT_ANCHOR"`）でも付けられるが、URL 欄とテキスト欄を両方埋める必要があり数が多いと現実的でない
...
| **全問の差し替え**（全問の訳語修正・出典のリンク化など、CSV 側を正として全部入れ替える） | **CSV 一括アップロードで置き換える**（ステップ6。テストは消さず、quizId・設定は残る）。投入後に編集画面ヘルパーの `auditBg` で全問を照合し、公開する。1本あたり数十秒で終わる |
| **多数の問題の書き換え**（一部の問題だけ、並びを変えずに直す） | **UI で問題ごとに書き換える。ヘルパーで自動化する（下記「多数の問題を書き換える」）**。1問あたり5〜10秒 |
