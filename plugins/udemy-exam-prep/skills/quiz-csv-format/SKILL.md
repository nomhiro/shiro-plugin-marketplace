...
**quiz.csv（Source of Truth）の出典は素の URL のまま書く。** 素の URL のまま一括アップロードすると
テキストとして保存され受講者はクリックできないので、投入の直前に `linkify_csv.py` で出典行の URL を
`<a href="URL">URL</a>` にした投入用 CSV を別に作る（[[udemy-bulk-upload]]「出典 URL はリンクにして投入する」）。
- タグ検査は `<a href="https://...">...</a>` の形だけを対象外にする（それ以外の `<単語>` は従来どおり FAIL）
- Markdown の `[text](url)` はリンクにならない（文字のまま残る）
- 出典行は `出典:`（または `参考:`）で始める。リンクにするのはこの行の URL だけ
- URL の直後に日本語を続けない。半角スペースか行末で区切る
